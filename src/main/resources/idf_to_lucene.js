/**
 * CSW 2.0.2 AP ISO 1.0 Record (full) to Lucene Document mapping according to
 * mapping IGC 1.0.3 Copyright (c) 2008 wemove digital solutions. All rights
 * reserved.
 * 
 * The following global variable are passed from the application:
 * 
 * @param recordNode
 *            A IDF record instance, that defines the input
 * @param document
 *            A lucene Document instance, that defines the output
 * @param log
 *            A Log instance
 * @param geometryMapper
 *            A de.ingrid.interfaces.csw.index.impl.IngridGeoTKLuceneIndexer
 *            instance
 * @param XPATH
 ...
 * transformation to be executed on the value funct: The transformation function
 * to use. params: The parameters for the transformation function additional to
 ...
 * other parameters are ignored. funct: The function to execute. params: The
 * parameters for the function additional to the recordNode that is always the
 * first parameter. tokenized: If set to false no tokenizing will take place
 * before the value is put into the index.
 */
var transformationDescriptions = [
		...
		// Additional queryable properties (dataset, datasetcollection,
		// application), OGC 07-045, Table 11, 12, 13
		{
...
function mapGeographicElements(recordNode) {
	...
	if (hasValue(boundingBoxes)) {
		...
		if (javaVersion.indexOf("1.8") === 0) {
			...
			westList = java.lang.reflect.Array.newInstance(java.lang.Double,
					boundingBoxes.getLength());
			eastList = java.lang.reflect.Array.newInstance(java.lang.Double,
					boundingBoxes.getLength());
			southList = java.lang.reflect.Array.newInstance(java.lang.Double,
					boundingBoxes.getLength());
			northList = java.lang.reflect.Array.newInstance(java.lang.Double,
					boundingBoxes.getLength());
		}
		for (i = 0; i < boundingBoxes.getLength(); i++) {
			if (hasValue(boundingBoxes.item(i))
					&& hasValue(XPATH.getString(boundingBoxes.item(i),
							"gmd:westBoundLongitude/gco:Decimal"))) {
				westList[i] = XPATH.getDouble(boundingBoxes.item(i),
						"gmd:westBoundLongitude/gco:Decimal");
				eastList[i] = XPATH.getDouble(boundingBoxes.item(i),
						"gmd:eastBoundLongitude/gco:Decimal");
				southList[i] = XPATH.getDouble(boundingBoxes.item(i),
						"gmd:southBoundLatitude/gco:Decimal");
				northList[i] = XPATH.getDouble(boundingBoxes.item(i),
						"gmd:northBoundLatitude/gco:Decimal");
			}
		}
		geometryMapper.addBoundingBox(document, westList, eastList, southList,
				northList, new java.lang.Integer(4326));
	}
}


function mapReferenceSystem(recordNode) {
	var referenceSystemIdentifiers = XPATH.getNodeList(recordNode,
			"//gmd:referenceSystemInfo//gmd:RS_Identifier");
	if (hasValue(referenceSystemIdentifiers)) {
		for (i = 0; i < referenceSystemIdentifiers.getLength(); i++) {
			var authority = XPATH.getString(referenceSystemIdentifiers.item(i),
					"gmd:codeSpace/gco:CharacterString");
			var id = XPATH.getString(referenceSystemIdentifiers.item(i),
					"gmd:code/gco:CharacterString");
			var version = XPATH.getString(referenceSystemIdentifiers.item(i),
					"gmd:version/gco:CharacterString");
			if (hasValue(authority) || hasValue(id) || hasValue(version)) {
				var crsString = "urn:ogc:def:objectType:" + authority + ":"
						+ version + ":" + id;
...
function mapHasSecurityConstraints(recordNode) {
	if (XPATH.nodeExists(recordNode,
			"//gmd:identificationInfo//gmd:MD_SecurityConstraints")) {
...
function addToDoc(field, content, tokenized) {
	if (typeof content != "undefined" && content != null) {
		...
		document.add(new Field("anytext", luceneTools.filterTerm(content),
...
function addNumericToDoc(field, content) {
	if (typeof content != "undefined" && content != null) {
		try {
			if (log.isDebugEnabled()) {
				log.debug("Add numeric '" + field + "'='" + content
						+ "' to lucene index.");
			}
			document.add(new NumericField(field, Field.Store.YES, true)
					.setDoubleValue(content));
		} catch (e) {
			if (log.isDebugEnabled()) {
				log.debug("Value '" + content
						+ "' is not a number. Ignoring field '" + field + "'.");
			}
		}
	}
}
