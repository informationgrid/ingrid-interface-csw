...
public class IngridQueryHelperTestLocal extends TestCase {
	...
	public void testGetCompleteAddress() throws Exception {
		...
		assertEquals((String)addressDetail.get(IngridQueryHelper.HIT_KEY_ADDRESS_INSTITUITION), "BAfM - Bundesanstalt fuer Milchforschung, Institut fuer Hygiene und Produktsicherheit");
		...
		assertEquals((String)addressDetail.get(IngridQueryHelper.HIT_KEY_ADDRESS_INSTITUITION), "AMilGeo - Amt fuer Militaerisches Geowesen");
		...
		addressDetail = (IngridHitDetail) address.get("detail");
		assertEquals((String)addressDetail.get(IngridQueryHelper.HIT_KEY_ADDRESS_INSTITUITION), "Bayrisches Landesamt fuer Denkmalpflege, Forschungsbereich Materialkonservierung");
		...
		addressDetail = (IngridHitDetail) address.get("detail");
		assertEquals((String)addressDetail.get(IngridQueryHelper.HIT_KEY_ADDRESS_INSTITUITION), "BGR - Bundesanstalt fuer Geowissenschaften und Rohstoffe, Fachgruppe 4.2 Mineralogie, Bodenkunde, Referat 4.24 Informationsgrundlagen im Boden- und Umweltschutz (Dienstbereich Berlin)");
		...
		addressDetail = (IngridHitDetail) address.get("detail");
		assertEquals((String)addressDetail.get(IngridQueryHelper.HIT_KEY_ADDRESS_INSTITUITION), "Behoerde fuer Inneres, Polizei Hamburg, Landespolizeiinspektion, Polizeiinspektion Sued, Fachabteilung Wasserschutzpolizei, Kriminalpolizeiliche Ermittlungsdienststellen (insb. PD 455)");
